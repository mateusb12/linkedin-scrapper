...
const SavedJobs = () => {
  ...
  // --- CLEANING UTILITY ---
  // This function removes the LinkedIn scraping "garbage" lines
  const cleanJobDescription = (rawText) => {
    ...
    // 2. Define garbage patterns to ignore
    const garbagePatterns = [
      ...
      /^start$/,
      /^strong$/,
      /^text-attr-\d+$/, // Matches text-attr-0, text-attr-1, etc.
      /^more$/,
    ...
    const cleanedLines = lines
      ...
      .filter((line) => {
        ...
        // Remove lines that match garbage patterns
        return !garbagePatterns.some((pattern) => pattern.test(line));
      })
      .map((line) => {
        // Optional: Replace 'li' with a bullet point if it appears alone
        if (line === "li" || line === "ul") return "• ";
        if (line === "br") return ""; // Skip explicit breaks (the newline handles it)
        return line;
      });
  ...
  const getInsightStyle = (text) => {
    ...
    if (t.includes("reviewing")) return "text-purple-300 bg-purple-900/20 border-purple-800/50";
    if (t.includes("viewed")) return "text-blue-300 bg-blue-900/20 border-blue-800/50";
    if (t.includes("applied")) return "text-emerald-300 bg-emerald-900/20 border-emerald-800/50";
    if (t.includes("posted")) return "text-indigo-300 bg-indigo-900/20 border-indigo-800/50";
    if (t.includes("no longer")) return "text-red-300 bg-red-900/20 border-red-800/50";
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                ...
                />
                <div className="absolute left-3 top-2.5 text-gray-500"><Search size={14} /></div>
              </div>
              <button onClick={loadJobs} disabled={isLoading} className="p-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-gray-300 transition-colors disabled:opacity-50">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-700">
              {isLoading ? (
                ...
                <tr><td colSpan="4" className="p-12 text-center text-red-400">{error}</td></tr>
              ) : filteredJobs.length > 0 ? (
                filteredJobs.map((job) => (
                  <React.Fragment key={job.job_posting_urn}>
                    <tr className={`group transition-colors ${expandedJobUrn === job.job_posting_urn ? "bg-gray-800" : "hover:bg-emerald-900/5"}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          <button onClick={() => toggleDescription(job.job_posting_urn)} className="mt-1 text-gray-500 hover:text-emerald-400 transition-colors focus:outline-none">
                            {expandedJobUrn === job.job_posting_urn ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                          </button>
                          <div>
                            <div className="font-bold text-gray-200 cursor-pointer hover:text-emerald-400" onClick={() => toggleDescription(job.job_posting_urn)}>
                        ...
                        <div className="flex flex-col gap-1 items-start">
                          {job.insights && job.insights.length > 0 ? (
                            job.insights.map((insight, idx) => (
                              <span key={idx} className={`px-3 py-1 rounded-full text-xs border whitespace-nowrap ${getInsightStyle(insight)}`}>
                                {insight.trim()}
                              </span>
                            ))
                          ) : (
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <a href={job.navigation_url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white hover:bg-gray-700 p-2 rounded-lg transition-colors">
                            <ExternalLink size={16} />
                          </a>
                          <button onClick={() => handleRemove(job.job_posting_urn)} className="text-gray-400 hover:text-red-400 hover:bg-red-900/20 p-2 rounded-lg transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
                    ...
                    {/* EXPANDABLE ROW WITH CLEANED TEXT */}
                      ...
                      <tr className="bg-gray-900/30 border-b border-gray-700/50 animate-in fade-in zoom-in-95 duration-200">
                        <td colSpan="4" className="px-6 py-4">
                          <div className="bg-gray-800 rounded-lg p-5 border border-gray-700 shadow-inner">
                            ...
                            <div className="text-gray-300 text-sm leading-relaxed whitespace-pre-line font-mono opacity-90 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-4 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="flex flex-col">
            <label className="text-xs text-gray-400 font-bold uppercase mb-1 ml-1 flex items-center gap-1"><Hash size={12} /> Copy First N Items</label>
            <input type="number" min="1" max={jobs.length} placeholder={`Max: ${jobs.length}`} value={exportCount} onChange={(e) => setExportCount(e.target.value)} className="bg-gray-900 border border-gray-600 text-white text-sm rounded-lg px-3 py-2 outline-none focus:border-emerald-500 w-32" />
          </div>
          <div className="flex flex-col justify-end h-full pt-5">
            <span className="text-sm font-medium text-gray-300 bg-gray-900 px-3 py-2 rounded-lg border border-gray-700 min-w-[100px] text-center">Count: <span className="text-emerald-400 font-bold">{jobsToExport.length}</span></span>
          </div>
        </div>
        <button onClick={handleCopyJobs} disabled={jobsToExport.length === 0} className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold transition-all duration-200 ${isCopied ? "bg-green-600 text-white" : jobsToExport.length === 0 ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg hover:shadow-emerald-500/20"}`}>
          {isCopied ? <><CheckCircle2 size={18} /> Copied!</> : <><Copy size={18} /> Copy {jobsToExport.length > 0 ? `(${jobsToExport.length})` : ""} JSON</>}
        </button>
      </div>
    </div>
  );

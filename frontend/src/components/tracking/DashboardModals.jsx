...
export const BackfillModal = ({ onClose, onComplete }) => {
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      `http://localhost:5000/pipeline/backfill-descriptions-stream?time_range=${timeRange}`,
    );
        ...
        <div className="p-6 border-b border-gray-800 bg-gray-900 flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <TrendingUp
                className={`text-blue-500 ${hasStarted && !isFinished ? "animate-pulse" : ""}`}
              />
              Enriching Job Data
            </h3>
            <p className="text-xs text-gray-500 mt-1">
              Updates applicants, descriptions & statuses.
            </p>
          </div>
          ...
          {!hasStarted ? (
            <div className="flex flex-col items-center justify-center py-10 space-y-4 text-center">
              ...
              <p className="text-sm text-gray-400 max-w-xs">
                Select a time range above and click Start. This will scan your
                saved jobs for updated applicant counts and statuses.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-800/50 p-3 rounded-lg border border-gray-700">
                  <div className="flex items-center gap-2 text-gray-400 text-xs mb-1">
                    <AlertCircle size={14} /> Updates Found
                  </div>
              ...
              >
                ...
                <p
                  className={`text-xs uppercase font-bold tracking-wider mb-1 ${isFinished ? "text-gray-500" : "text-blue-400"}`}
                >
              ...
              <div className="space-y-2 bg-black/20 p-3 rounded-lg border border-gray-800 max-h-48 overflow-y-auto">
                ...
                {logs.map((log) => (
                  <div
                    key={log.id}
                    className="text-xs border-b border-gray-800/50 pb-2 last:border-0 last:pb-0"
                  >
                    ...
                    {log.changes && log.changes.length > 0 ? (
                      <div className="pl-5 space-y-1">
                        {log.changes.map((change, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-1.5 text-gray-400 font-mono text-[10px]"
                          >
                      ...
                      <div className="pl-5 text-[10px] text-gray-600 italic">
                        No changes detected
                      </div>
                    )}
                  </div>
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (liAtMatch && jsessionMatch) {
          ...
          setStatusMessage("❌ Error: Could not find li_at or JSESSIONID.");
        }
      } catch (err) {
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8 border border-gray-700 animate-in fade-in slide-in-from-top-4">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            <Terminal size={20} className="text-blue-400" /> Update LinkedIn
            Credentials
          </h3>
        </div>
      ...
      <div className="border-2 border-dashed border-gray-600 rounded-lg p-8 bg-gray-900/50 flex flex-col items-center">
        ...
        >
          <Upload size={18} /> Upload .HAR File
        </button>
        {statusMessage && (
          <div className="mt-4 text-sm text-gray-300">{statusMessage}</div>
        )}
      </div>
    </div>
  );

...
import {
    Users, AlertTriangle, CheckCircle, Calendar, Filter, ArrowUpRight,
    Activity, PieChart, BarChart3, Eye, Ban, GitMerge,
    Minus, Plus // Added these for the picker
} from 'lucide-react';
...
// --- Styled Stat Card (Kept same as yours) ---
...
const BucketStepper = ({ value, onChange, min = 2, max = 20 }) => {
    ...
    return (
        <div className="flex items-center gap-2 bg-gray-900/80 p-1 rounded-lg border border-gray-700/50">
            <span className="text-[10px] text-gray-500 uppercase font-bold pl-2 pr-1">Buckets</span>
...
const DashboardInsights = ({ insights, rawJobs, timeRange, onTimeRangeChange }) => {
    const safeInsights = insights || {};
    const overview = safeInsights.overview || { total: 0, refused: 0, waiting: 0, reviewing: 0, closed: 0 };
    const rawComp = safeInsights.competition || {};

    const [bucketCount, setBucketCount] = useState(6); // Default 6 buckets
    ...
    // --- Dynamic Histogram Logic (Optimized for Outliers) ---
    const competitionData = useMemo(() => {
        if (!rawJobs || rawJobs.length === 0) return { labels: [], datasets: [] };

        // 1. Get raw values
        const validJobs = rawJobs.filter(j => j.applicants !== undefined && j.applicants !== null);
        if (validJobs.length === 0) return { labels: [], datasets: [] };

        const values = validJobs.map(j => parseInt(j.applicants)).sort((a, b) => a - b);

        // 2. Calculate P95 (95th Percentile) to handle outliers
        // This ensures the chart scales to the majority of data, not the one job with 10k applicants
        const p95Index = Math.floor(values.length * 0.95);
        const p95Value = values[p95Index];
        const minVal = values[0];

        // Use P95 as the "Max" for the main buckets.
        // Anything above P95 goes into a special "Overflow" bucket.
        // We use (bucketCount - 1) because the last bucket is reserved for overflow
        ...
        validJobs.forEach(job => {
            ...
            // If value exceeds our calculated range, put it in the last bucket
            ...
            datasets: [
                {
                    ...
                    backgroundColor: buckets.map((b, i) => {
                        // Use a distinct color for the overflow bucket to indicate it's special
                        if (b.isOverflow) return '#fbbf24'; // Amber for high competition/outliers
                        const colors = ['#34d399', '#60a5fa', '#818cf8', '#a78bfa', '#e879f9', '#f472b6'];
                ...
                },
                {
                    ...
                    showLine: false,
                    label: 'Refusal Rate %',
                    data: buckets.map(b => b.count > 0 ? ((b.refused / b.count) * 100).toFixed(1) : 0),
    ...
    // --- Other Charts Data (Donut, Sankey) kept standard ---
    const statusData = {
    ...
    const sankeyOptions = {
        sankey: {
            node: {
                colors: ["#3b82f6", "#10b981", "#6b7280", "#fbbf24", "#a855f7", "#374151", "#ef4444"],
                label: { fontName: 'Inter', fontSize: 13, color: '#e5e7eb', bold: false },
                nodePadding: 30, width: 6,
            },
            link: { colorMode: 'gradient', colors: ["#3b82f6", "#6b7280", "#fbbf24", "#a855f7", "#374151", "#ef4444"] }
        },
    ...
    return (
        <div className="space-y-6 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center bg-gray-800/40 p-4 rounded-2xl border border-gray-700/50 backdrop-blur-md">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">Deep dive into application states</p>
                    </div>
                </div>
                <div className="mt-4 sm:mt-0 flex items-center gap-3 bg-gray-900 p-1 rounded-xl border border-gray-700/50 shadow-inner">
                    ...
                    <select value={timeRange} onChange={(e) => onTimeRangeChange(e.target.value)} className="bg-gray-900 border-none text-gray-300 text-sm focus:ring-0 cursor-pointer py-1.5 pr-8 pl-2 font-medium hover:text-white transition-colors rounded-r-xl outline-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                <StatCard title="Active Eyes" value={overview.reviewing} icon={Eye} color="purple" iconColor="text-purple-400" subtext={`${reviewRate}% of total apps`} />
                <StatCard title="Total Active" value={activePipeline} icon={CheckCircle} color="yellow" iconColor="text-yellow-400" subtext="Waiting + Reviewing" />
                <StatCard title="Dead Ends" value={deadEnds} icon={Ban} color="gray" iconColor="text-gray-400" subtext="Closed or Refused" />
                <StatCard title="Avg Applicants" value={rawComp.avg_applicants} icon={Users} color="blue" iconColor="text-blue-400" subtext="Candidates per role" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-80">
                ...
                <div className="bg-gray-800/60 backdrop-blur-sm p-6 rounded-2xl border border-gray-700/50 flex flex-col shadow-lg">
                    <div className="flex justify-between items-center mb-4 h-8">
                        ...
                        <div className="text-xs text-gray-500 font-mono bg-gray-900/50 px-2 py-1 rounded border border-gray-700/30">Total: {overview.total}</div>
                    </div>
                    <div className="flex-1 flex items-center justify-between gap-2 min-h-0">
                        ...
                        <div className="w-1/2 flex flex-col justify-center space-y-3 pr-2 text-sm">
                            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-purple-500"></span><span className="text-gray-300">Reviewing</span></div><span className="font-bold text-white">{overview.reviewing}</span></div>
                            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-yellow-400"></span><span className="text-gray-300">Waiting</span></div><span className="font-bold text-white">{overview.waiting}</span></div>
                            <div className="w-full h-px bg-gray-700/50 my-1"></div>
                            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-gray-600"></span><span className="text-gray-400">Closed</span></div><span className="font-bold text-gray-400">{overview.closed}</span></div>
                            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-red-400"></span><span className="text-gray-400">Refused</span></div><span className="font-bold text-gray-400">{overview.refused}</span></div>
                        </div>
                    </div>
                </div>

                {/* 2. DYNAMIC HISTOGRAM (Fixed) */}
                <div className="bg-gray-800/60 backdrop-blur-sm p-6 rounded-2xl border border-gray-700/50 flex flex-col shadow-lg">
                    <div className="flex justify-between items-center mb-4 h-8">
                        <div className="flex items-center gap-2">
                            <div className="p-1.5 bg-gray-700/50 rounded-md text-gray-400"><BarChart3 size={14}/></div>
                            <h4 className="font-bold text-white text-sm">Competition Distribution</h4>
                        </div>
                        {/* New Stepper Picker */}
            ...
            {overview.total > 0 && (
                <div className="bg-gray-800/60 backdrop-blur-sm p-6 rounded-2xl border border-gray-700/50 shadow-lg min-h-[400px] flex flex-col">
                    ...
                    <div className="flex-1 w-full h-full min-h-[350px]">
                        <Chart chartType="Sankey" width="100%" height="100%" data={filteredSankeyData} options={sankeyOptions} />
                    </div>
                </div>
            )}
        </div>

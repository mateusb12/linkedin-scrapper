...
const StreakCalendar = ({ dailyStats }) => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    calendarDays.push(
      new Date(
        Date.UTC(currentMonth.getUTCFullYear(), currentMonth.getUTCMonth(), i),
      ),
    );
  }
    ...
    <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-xl mt-6">
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div
            key={d}
            className="text-center text-xs text-gray-500 font-bold uppercase"
          >
            {d}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 gap-2">
        {calendarDays.map((date, idx) => {
          ...
          let baseClass =
            "h-12 w-full rounded-lg flex flex-col items-center justify-center border transition-all relative group";
          let content = null;
          ...
          if (isFuture) {
            ...
            baseClass +=
              " bg-gradient-to-br from-amber-400 to-amber-600 border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.3)] transform hover:scale-105 z-10";
          ...
          return (
            <div key={idx} className={baseClass}>
              <span
                className={`absolute top-1 left-1.5 text-[10px] font-bold ${numberColor}`}
              >
                {date.getUTCDate()}
              </span>
              {content}

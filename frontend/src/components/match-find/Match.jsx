    ...
    return (
        <div className="pt-8">
            ...
            <div className="space-y-6">
                <div
                    ...
                    {skillsChanged ? (
                        <div
                            ...
                            <div>
                                <label
                                    className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">ORIGINAL</label>
                                <p className="text-sm text-gray-800 dark:text-gray-200 bg-gray-200 dark:bg-gray-800/90 p-2 rounded-md">{originalSkillsText}</p>
                            </div>
                            <div>
                                <label htmlFor="edited-skills"
                                       className="block text-xs font-semibold text-purple-800 dark:text-purple-300 mb-1">SUGGESTED
                                    (EDITABLE)</label>
                                <textarea
                                    id="edited-skills"
                                    value={editedSkills}
                ...
                <div
                    className="p-4 border-2 border-dashed border-purple-300 dark:border-purple-800 rounded-lg bg-purple-50 dark:bg-purple-900/10">
                    <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">Professional
                        Experience</h3>
...
const JobDetailView = ({job, resume, onMarkAsApplied}) => {
    ...
    const Placeholder = ({text = "None specified"}) => <div
        className="flex items-center text-gray-500 dark:text-gray-400 italic"><XCircle size={16}
                                                                                       className="mr-2"/><span>{text}</span>
    </div>;
    const DetailSection = ({title, icon, items}) => (
        <div>
            ...
            {items && items.length > 0 ?
                <ul className="space-y-2 list-disc list-inside text-gray-800 dark:text-gray-300">{items.map((item, index) =>
    ...
    return (
        <div className="p-6 md:p-8 h-full overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 text-sm">
                {job.job_type && (
                    <div
                        className="flex items-center text-indigo-900 dark:text-indigo-100 bg-indigo-100 dark:bg-indigo-900/40 p-3 rounded-lg border-l-4 border-indigo-500 dark:border-indigo-400 shadow-sm">
                ...
                {Array.isArray(job.programming_languages) && job.programming_languages.length > 0 && (
                    <div
                        className="flex items-center bg-violet-100 dark:bg-violet-900/40 p-3 rounded-lg border-l-4 border-violet-500 dark:border-violet-400 shadow-sm flex-wrap gap-2 text-violet-900 dark:text-violet-100">
                        ...
                        {job.programming_languages.map((lang, index) => {
                            ...
                            return (
                                <span
                                    key={index}
                                    className="flex items-center gap-2 bg-violet-200 dark:bg-violet-700 text-violet-900 dark:text-violet-100 px-3 py-1 rounded-full text-sm font-bold"
                                >
            {icon}
                                    {lang}
        </span>
                ...
                <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-3 rounded-lg"><MapPin size={18}
                                                                                                       className="mr-3 text-gray-500"/>{job.location}
                </div>
                <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-3 rounded-lg"><Briefcase size={18}
                                                                                                          className="mr-3 text-gray-500"/>{job.employment_type}
                </div>
                <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-3 rounded-lg"><Clock size={18}
                                                                                                      className="mr-3 text-gray-500"/>Posted: {formatDate(job.posted_on)}
                </div>
                <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-3 rounded-lg"><Building size={18}
                                                                                                         className="mr-3 text-gray-500"/>{job.workplace_type}
                </div>
            </div>
          ...
          <CheckCircle size={14}/> {keyword}
                ...
                <DetailSection title="Qualifications" icon={<ListChecks size={20} className="mr-2"/>}
...
const Match = () => {
    ...
    const StatusIndicator = () => {
        if (status === 'idle' && matchedJobs.length === 0) return <div className="p-8 text-center text-gray-500"><Target
            size={48} className="mx-auto mb-4 text-gray-400"/><h3 className="text-xl font-semibold">Ready to Match</h3>
            <p>Select your resume and click "Find Best Matches".</p></div>;
        if (status === 'matching') return <div className="p-8 text-center text-gray-500">
            <svg className="animate-spin mx-auto h-12 w-12 text-sky-500" xmlns="http://www.w3.org/2000/svg" fill="none"
                 viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor"
                      d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="mt-4">Finding the best opportunities for you...</p></div>;
        if (status === 'success' && matchedJobs.length === 0) return <div className="p-8 text-center text-gray-500"><h3
            className="text-xl font-semibold">No Matching Jobs Found</h3><p>We couldn't find any jobs that match the
            skills in the selected resume.</p></div>;
        return null;
    };
    ...
    return (
        <div className="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
            <div className="flex h-screen">
                <div
                    className="flex flex-col flex-shrink-0 w-[35%] max-w-md border-r border-gray-200 dark:border-gray-700">
                    <div className="p-4 border-b border-gray-200 dark:border-gray-700 space-y-4">
                        <h2 className="text-xl font-bold flex items-center gap-2"><Award size={24}
                                                                                         className="text-sky-500"/> Job
                            Matcher</h2>
                        <div>
                            <label htmlFor="resume-select"
                                   className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">1. Select
                                your resume</label>
                            <select id="resume-select" value={selectedResumeId}
                                ...
                                {resumes.map(resume => <option key={resume.id}
                        ...
                        {jobMetrics.total > 0 && <div
                            ...
                            className="text-yellow-600 dark:text-yellow-400">{jobMetrics.incomplete} incomplete</span>
                        </p></div>}
                        <div>
                            <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">2. Find
                                your matches</label>

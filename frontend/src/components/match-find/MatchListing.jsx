            ...
            <div className="flex justify-between items-start">
                <div>
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                        ...
                        <span>{job.title}</span>
                        {job.easy_apply && <Zap size={14} className="text-yellow-500 flex-shrink-0" title="Easy Apply" />}
                    </h3>
...
const JobListing = ({
    ...
    const filteredMatchedJobs = useMemo(() => matchedJobs.filter(job =>
        !selectedLanguageFilter || (job.programming_languages || []).map(l => l.toLowerCase()).includes(selectedLanguageFilter)
            ...
            <div className="p-4 border-b border-gray-200 dark:border-gray-700 space-y-4">
                ...
                {matchedJobs.length > 0 && (
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-2 text-center">
                        Showing <strong>{filteredMatchedJobs.length}</strong> of <strong>{matchedJobs.length}</strong> matched jobs
                    </p>
                )}

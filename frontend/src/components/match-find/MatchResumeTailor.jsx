...
const AdaptResumeSection = ({ baseResume, job, allResumes, onSelectResume, profile }) => {
    ...
    useEffect(() => {
        if (baseResume) {
            ...
            if (mappedResume.professional_experience) { mappedResume.professional_experience.forEach(exp => { exp.details = exp.description || []; }); }
            if (mappedResume.projects) { mappedResume.projects.forEach(proj => { proj.details = proj.description || []; }); }

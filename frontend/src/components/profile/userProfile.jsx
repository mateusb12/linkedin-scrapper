...
const ProfileDetails = ({
  ...
  return (
    <div className={`${palette.bg.card} p-6 rounded-lg shadow-lg`}>
      <h2
        className={`text-2xl font-bold ${palette.text.light} mb-6 border-b ${palette.border.primary} pb-3`}
      >
        👤 Profile Details
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          <div className="flex items-center gap-1 pb-1">
            <Globe className={`h-${iconSize} w-${iconSize}`} /> Portfolio URL
          </div>
          <input
            type="text"
            name="portfolio"
      ...
      <div className="space-y-4 mt-6 pt-6 border-t border-gray-700">
        <h3 className={`text-xl font-semibold ${palette.text.primary}`}>
          Education
        </h3>
        {(profile.education || []).map((edu, index) => (
          <div
            key={edu.id || index}
            className={`${palette.bg.nestedCard} p-4 rounded-md border ${palette.border.secondary} relative`}
          >
            <button
...
const ResumeSection = ({
  ...
  if (!resumes || resumes.length === 0)
    return (
      <div
        className={`${palette.bg.card} p-6 rounded-lg shadow-lg mt-8 text-center ${palette.text.secondary}`}
      >
  ...
  const removeNestedItem = (index, section) => {
    const updatedSection = (selectedResume[section] || []).filter(
      (_, i) => i !== index,
    );
    handleResumeFieldChange(section, updatedSection);
  };
  ...
  return (
    <div className={`${palette.bg.card} p-6 rounded-lg shadow-lg mt-8`}>
      <div
        className={`flex flex-col md:flex-row justify-between md:items-center mb-6 border-b ${palette.border.primary} pb-3`}
      >
        <h2 className={`text-2xl font-bold ${palette.text.light} mb-3 md:mb-0`}>
          📄 Resumes
        </h2>
        <select
      ...
      {selectedResume && (
        <div>
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleResumeFieldChange("name", e.target.value)
                }
                className={inputClasses}
              />
            </div>
            <div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleResumeFieldChange("summary", e.target.value)
                }
                className={inputClasses}
                rows="5"
              />
            ...
            <div>
              <DynamicInputSection
                ...
                setItems={(newSkills) =>
                  handleResumeFieldChange("hard_skills", newSkills)
                }
              />
              <MarkdownPreview
                sectionTitle="Hard Skills"
                markdownContent={generateHardSkillsMarkdown(
                  selectedResume.hard_skills || [],
                )}
              />
            </div>
            ...
            <div className="space-y-4">
              <h3
                className={`text-xl font-semibold ${palette.text.primary} pt-4 border-t ${palette.border.primary}`}
              >
              ...
              {(selectedResume.professional_experience || []).map(
                (exp, index) => (
                  ...
                  >
                    <button
                      onClick={() =>
                        removeNestedItem(index, "professional_experience")
                      }
                      className={styleguide.iconButton.delete}
                    >
                      <svg
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          const newDesc = [...(exp.description || []), ""];
                          handleNestedChange(
                            { target: { name: "description", value: newDesc } },
                            index,
                            "professional_experience",
                          );
                        }}
              ...
              <button
                onClick={() => addNestedItem("professional_experience")}
                className={styleguide.button.success}
              >
                + Add Experience
              </button>
              <MarkdownPreview
                ...
                markdownContent={generateExperienceMarkdown(
            ...
            <div className="space-y-4">
              ...
              {(selectedResume.projects || []).map((proj, index) => (
                ...
                >
                  <button
                    onClick={() => removeNestedItem(index, "projects")}
                    className={styleguide.iconButton.delete}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                    ...
                    {(proj.description || []).map((point, dIndex) => (
                      <div key={dIndex} className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const newDesc = proj.description.filter(
                              (_, i) => i !== dIndex,
                            );
                            handleNestedChange(
                              {
              ...
              <MarkdownPreview
                ...
                markdownContent={generateProjectsMarkdown(
          ...
          <div
            className={`flex flex-col sm:flex-row justify-end items-center mt-6 pt-6 border-t ${palette.border.primary} space-y-3 sm:space-y-0 sm:space-x-4`}
          >
            <button
            ...
            <button
              ...
              className="border border-red-600 text-red-500 hover:text-white hover:bg-red-600 font-bold py-2 px-4 rounded-md transition w-full sm:w-auto"
            >
              Delete Resume
            </button>
            <button
...
const UserProfile = () => {
  ...
  const loadDataFromMarkdown = async () => {
    if (
      !window.confirm(
        "This will replace current data with the content from public/myProfile.md. Are you sure?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`Failed to fetch myProfile.md: ${response.statusText}`);

      const markdownText = await response.text();
      const { data: profileData, content: resumesString } =
        matter(markdownText);
      ...
      const parsedResumes = rawParsedResumes
        ...
        .map((resume) => ({
          ...resume,
          professional_experience: (resume.professional_experience || []).map(
            (exp, index) => ({ ...exp, id: `exp_${resume.id}_${index}` }),
          ),
  ...
  const handleSaveResume = async () => {
    ...
    try {
      ...
      if (existingResume && existingResume.id) {
        rawSavedResume = await updateResume(existingResume.id, {
          ...selectedResume,
          id: existingResume.id,
        });
      } else {
        rawSavedResume = await createResume(selectedResume);
      }

      const savedResume = {
        ...rawSavedResume,
        hard_skills: rawSavedResume.hard_skills || [],
        professional_experience: rawSavedResume.professional_experience || [],
        projects: rawSavedResume.projects || [],
        education: rawSavedResume.education || [],
      };

      setResumes((prevResumes) =>
        prevResumes.map((r) => (r.id === selectedResume.id ? savedResume : r)),
      );
  ...
  const generateFullResumeMarkdown = () => {
    ...
    const experience = generateExperienceMarkdown(
      selectedResume.professional_experience,
    );
    ...
    return [header, skills, experience, education, projects]
      .filter(Boolean)
      .join("\n\n---\n\n");
  };
  ...
  if (loading)
    return (
      <div
        className={`${palette.bg.page} ${palette.text.light} min-h-screen flex items-center justify-center`}
      >
        <p className="text-xl">Loading data...</p>
      </div>
    );
  if (error && !profile.id && resumes.length === 0)
    return (
      <div
        className={`${palette.bg.page} ${palette.text.light} min-h-screen flex items-center justify-center`}
      >
        <div className="text-center p-8 bg-gray-800 rounded-lg shadow-lg">
          ...
          <button
            onClick={loadDataFromMarkdown}
            className={styleguide.button.secondary}
          >
    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-wrap justify-between items-center gap-4 mb-8">
          <h1 className={`text-4xl font-extrabold ${palette.text.light}`}>
            Edit Profile & Resumes
          </h1>
          <button
            onClick={loadDataFromMarkdown}
            className={styleguide.button.secondary}
            title="Load data from the public/myProfile.md file"
          >
            Load from Markdown
          </button>
        </div>
        ...
        {showFullPreview && (
          <div className={`mt-8 ${palette.bg.card} p-6 rounded-lg shadow-lg`}>
            <div
              className={`flex justify-between items-center mb-4 pb-3 border-b ${palette.border.primary}`}
            >
